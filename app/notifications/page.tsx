...
export default function NotificationsPage() {
  ...
  const handleNotificationClick = (notification: any) => {
    ...
    $api.delete(`${API_URL}/notification?id=${notification.id}`).then((res) => {
  ...
  const markAllAsRead = () => {
    $api.delete(`${API_URL}/notification`).then((res) => {
  ...
  const deleteNotifications = (id: string) => {
    $api.delete(`${API_URL}/notification?id=${id}`).then((res) => {
      window.location.reload();
    });
  };
      ...
      <main className="flex-1 container py-8">
        <div className="flex items-center gap-2">
          <Button
            variant="ghost"
            size="sm"
            className="gap-1 p-0 h-auto mb-6"
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-muted-foreground">
                Управляйте своими уведомлениями и будьте в курсе последних
                обновлений
              </p>
            </div>
